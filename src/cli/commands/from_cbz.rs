...
pub fn execute(args: FromCbzArgs) -> Result<()> {
    ...
    let mut archive =
        ZipArchive::new(file).context("Failed to read CBZ archive - file may be corrupted")?;

    let mut entries = collect_image_entries(&mut archive)?;
    ...
    let temp_files = extract_to_temp(&mut archive, &entries, &temp_dir)?;
